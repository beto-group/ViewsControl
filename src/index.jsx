      ...
      const load = async () => {
        try {
          ...
          const [
            ...
            dc.require(base + "/src/components/ScreenModeHelper.jsx"),
            dc.require(base + "/src/App.jsx")
          ]);
          setModules({
            ScreenModeHelper: helper.ScreenModeHelper,
            WorldView: app.WorldView
          });
        } catch (e) {
          setError(e);
        }
    ...
    if (!modules) {
      return (
        <div style={{ padding: "40px", background: "#000", color: "#666", height: "100vh", fontFamily: "monospace" }}>
          Initializing Views Control...
        </div>
      );
    }

    const { WorldView, ScreenModeHelper } = modules;
    return (
      <div id="datacore-component-root" style={{ width: "100%", height: "100%" }}>
        <WorldView folderPath={folderPath} dc={dc} ScreenModeHelper={ScreenModeHelper} />
      </div>
    );
  };
